...
static BOOL ShouldDownloadOrDelete(BOOL bInstall, HINSTANCE hInstance, WCHAR const *wszPath, char const *chash);
static BOOL InstallResource(BOOL bInstall, HMODULE hModule, int res, WCHAR const *wszPath);
static BOOL SetupShortcut(BOOL bInstall, WCHAR const *wszPath, WCHAR const *wszArguments);
static BOOL SetupUninstallEntry(BOOL bInstall, WCHAR* wszPath);
static void simpleInstall(int argc, wchar_t *wargv[], HINSTANCE hInstance);
...
static BOOL ShouldDownloadOrDelete(BOOL bInstall, HINSTANCE hInstance, WCHAR const *wszPath, char const *chash)
{
    if (FileExistsW(wszPath)) {
        ...
        ComputeFileHash(wszPath, hash, sizeof hash);
        if (!StrIEq(hash, chash)) {
            if (bInstall)
                return TRUE;
        } else {
            if (!bInstall)
...
static BOOL SetupShortcut(BOOL bInstall, WCHAR const *wszPath, WCHAR const *wszArguments)
{
    ...
    if (bInstall) {
        ...
        if (hExplorerFrame) {
            ...
            if (wszTitle[0] == 0)
                wcscat_s(wszTitle, MAX_PATH, L"" PRODUCT_NAME);
        } else {
            wcscat_s(wszTitle, MAX_PATH, L"" PRODUCT_NAME);
        }
    }
    ...
    wcscat_s(wszStartPrograms, MAX_PATH + 1, L"\\" PRODUCT_NAME);
    ...
    wcscat_s(wszStartPrograms, MAX_PATH, L" (");
    wcscat_s(wszStartPrograms, MAX_PATH, PRODUCT_NAME L").lnk");
    ZeroMemory(wszExplorerPath, MAX_PATH);
    GetSystemDirectoryW(wszExplorerPath, MAX_PATH);
    ...
    if (bInstall) {
        if (SUCCEEDED(CoInitialize(0))) {
            ...
            if (SUCCEEDED(CoCreateInstance(&CLSID_ShellLink, NULL, CLSCTX_INPROC, &IID_IShellLinkW, &pShellLink))) {
                ...
                pShellLink->lpVtbl->SetDescription(pShellLink, L"" PRODUCT_NAME);
    ...
    if (bInstall)
    {
        ...
        if (!dwLastError)
        {
            dwLastError = RegCreateKeyExW(
                HKEY_LOCAL_MACHINE,
                L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\" EP_CLSID L"_" PRODUCT_NAME,
            ...
            if (hKey)
            {
                ...
                if (!dwLastError)
                {
                    ...
                    wcscat_s(wszPath + 1, MAX_PATH - 2, L"\\" PRODUCT_NAME L".amd64.dll");
...
static void simpleInstall(int argc, wchar_t *wargv[], HINSTANCE hInstance)
{
    ...
    BOOL bOk;
    //wcscat_s(wszPath, MAX_PATH, L"\\" PRODUCT_NAME L".IA-32.dll");
    ...
    if (argc >= 2)
        ...
        GetCurrentDirectoryW(MAX_PATH, wszPath);
    //if (bOk) {
        wcscat_s(wszPath, MAX_PATH, L"\\" PRODUCT_NAME L".amd64.dll");
    ...
    if (bOk) {
        ...
        InstallResource(TRUE, hInstance, IDR_EP_STARTMENU, wszPath);
    }
}

/****************************************************************************************/

static HANDLE getExplorerProcessToken(HWND hShellTrayWnd)
{
    ...
    if (explorerProcessId != 0) {
        ...
        if (explorerProcess && explorerProcess != INVALID_HANDLE_VALUE) {
            HANDLE userToken;
    ...
    return INVALID_HANDLE_VALUE; // Error
}

static TOKEN_USER *getTokenUser(HANDLE token)
{
    DWORD cbSizeNeeded = 0;
    SetLastError(0);
    if (GetTokenInformation(token, TokenUser, NULL, 0, &cbSizeNeeded) || GetLastError() != ERROR_INSUFFICIENT_BUFFER)
        return NULL;
    TOKEN_USER *tokenInfo = malloc(cbSizeNeeded);
    if (!tokenInfo || !GetTokenInformation(token, TokenUser, tokenInfo, cbSizeNeeded, &cbSizeNeeded)) {
        free(tokenInfo);
        return NULL;
    }
    return tokenInfo;
...
static BOOL checkTokenSID(HANDLE userToken)
{
    static DWORD const desiredAccess = TOKEN_QUERY | TOKEN_DUPLICATE | TOKEN_ASSIGN_PRIMARY;
    HANDLE myToken = INVALID_HANDLE_VALUE;
    if (!OpenProcessToken(GetCurrentProcess(), desiredAccess, &myToken) || myToken == INVALID_HANDLE_VALUE)
        return FALSE;

    TOKEN_USER *userTokenInfo = getTokenUser(userToken);
    TOKEN_USER *myTokenInfo   = getTokenUser(myToken);
    BOOL        result;

    if (userTokenInfo && myTokenInfo)
        result = EqualSid(userTokenInfo->User.Sid, myTokenInfo->User.Sid);
    else
        result = FALSE;
...
)
{
    ...
    if (argc >= 1 && WStrIEq(wargv[0], L"/extract")) {
        simpleInstall(argc, wargv, hInstance);
        exit(0);
    }
    ...
    BOOL bForcePromptForUninstall = FALSE;
    BOOL bInstall = !(argc >= 1 && (WStrIEq(wargv[0], L"/uninstall") || WStrIEq(wargv[0], L"/uninstall_silent")));

    PathStripPathW(wszOwnPath);
    if (WStrIEq(wszOwnPath, L"ep_uninstall.exe")) {
        bInstall = FALSE;
    ...
    BOOL bIsUpdate = (argc >= 1 && WStrIEq(wargv[0], L"/update_silent"));
    if (!bInstall && (WStrIEq(wargv[0], L"/uninstall") || bForcePromptForUninstall) &&
        MessageBoxW(NULL, L"Are you sure you want to remove " PRODUCT_NAME L" from your computer?",
                    L"" PRODUCT_NAME, MB_YESNO | MB_DEFBUTTON2 | MB_ICONQUESTION
        ) == IDNO)
    {
        exit(0);
    }
    ...
    if (!IsAppRunningAsAdminMode()) {
        SHELLEXECUTEINFOW sei = {
            ...
            .lpVerb       = L"runas",
            .lpFile       = wszOwnPath,
            .lpParameters = !bInstall ? L"/uninstall_silent" : lpCmdLine,
            .nShow        = SW_NORMAL,
        };
    ...
    // BUG WTF is this? Just make an event and discard it?
    CreateEventW(NULL, FALSE, FALSE, L"" EP_SETUP_EVENTNAME);
    ...
    DWORD dwError;
    if (!CreateDirectoryW(wszPath, NULL) && (dwError = GetLastError()) != ERROR_ALREADY_EXISTS)
        return (int)dwError;

    HANDLE userToken     = INVALID_HANDLE_VALUE;
    HWND   hShellTrayWnd = FindWindowW(L"Shell_TrayWnd", NULL);
    if (hShellTrayWnd) {
        userToken = getExplorerProcessToken(hShellTrayWnd);
        if (userToken && checkTokenSID(userToken)) {
    ...
    HANDLE h_exists = CreateEventW(NULL, FALSE, FALSE, L"Global\\ep_dwm_" EP_CLSID);
    if (h_exists) {
    ...
    SHELLEXECUTEINFO ShExecInfo = {
        ...
        .lpVerb       = L"runas",
        .lpFile       = wszSCPath,
        .lpParameters = L"stop " EP_DWM_SERVICENAME,
    };
    ...
    HWND hWnd = FindWindowW(L"ExplorerPatcher_GUI_" EP_CLSID, NULL);
    if (hWnd) {
        DWORD dwGUIPid = 0;
        ...
        if (dwGUIPid) {
            ...
            if (hProcess) {
                ...
                if (hKey == NULL || hKey == INVALID_HANDLE_VALUE)
                    hKey = NULL;
                if (hKey) {
                    RegSetValueExW(hKey, L"OpenPropertiesAtNextStart", 0,
                                   REG_DWORD, &dwSection, sizeof(DWORD));
                    RegCloseKey(hKey);
                }
            }
        }
    ...
    BOOL bWasShellExt = FALSE;
    SHGetFolderPathW(NULL, SPECIAL_FOLDER, NULL, SHGFP_TYPE_CURRENT, wszPath);
    wcscat_s(wszPath, MAX_PATH, APP_RELATIVE_PATH L"\\" SETUP_UTILITY_NAME);
    BOOL bOk = InstallResource(bInstall, hInstance, 0, wszPath);
    if (bOk) {
        if (!bInstall) {
            HKEY hKey;
            RegOpenKeyExW(
    ...
    if (bOk) {
        ...
        wcscat_s(wszPath, MAX_PATH, APP_RELATIVE_PATH L"\\" PRODUCT_NAME L".amd64.dll\",ZZGUI");
        pArgs[0] = 0;
    ...
    if (bOk) {
        ...
        wcscat_s(wszPath, MAX_PATH, APP_RELATIVE_PATH L"\\" SETUP_UTILITY_NAME L"\" /uninstall");
        bOk = SetupUninstallEntry(bInstall, wszPath);
    }

    ShExecInfo.lpParameters = bInstall ? L"start " EP_DWM_SERVICENAME : L"delete " EP_DWM_SERVICENAME;
    if (ShellExecuteExW(&ShExecInfo) && ShExecInfo.hProcess) {
        WaitForSingleObject(ShExecInfo.hProcess, INFINITE);
        DWORD dwExitCode = 0;
        GetExitCodeProcess(ShExecInfo.hProcess, &dwExitCode);
    ...
    if (bOk) {
        ...
        wcscat_s(wszArgs, MAX_PATH, APP_RELATIVE_PATH L"\\ep_weather_host.dll\"");
        wprintf(L"%s\n", wszArgs);
    ...
    if (bOk) {
        ...
        SHGetFolderPathW(NULL, SPECIAL_FOLDER, NULL, SHGFP_TYPE_CURRENT, wszArgs + 4 + (bInstall ? 0 : 3));
        wcscat_s(wszArgs, MAX_PATH, APP_RELATIVE_PATH L"\\ep_weather_host_stub.dll\"");
        wprintf(L"%s\n", wszArgs);
    ...
    if (!bInstall) {
        if (bOk) {
            SHGetFolderPathW(NULL, SPECIAL_FOLDER, NULL, SHGFP_TYPE_CURRENT, wszPath);
            wcscat_s(wszPath, MAX_PATH, APP_RELATIVE_PATH L"\\ep_weather_host.dll");
            bOk = InstallResource(bInstall, hInstance, IDR_EP_WEATHER, wszPath);
        }
        if (bOk) {
            PathRemoveFileSpecW(wszPath);
    ...
    if (bOk) {
        if (!bInstall) {
            if (bWasShellExt) {
                if (MessageBoxW(NULL, L"Please reboot the computer to complete the uninstall.\n\nDo you want to reboot now?",
                                L""  PRODUCT_NAME, MB_YESNO | MB_DEFBUTTON1 | MB_ICONQUESTION
                    ) == IDYES)
                {
                    SystemShutdown(TRUE);
                }
            } else {
                MessageBoxW(NULL, L"Uninstall completed. Thank you for using " PRODUCT_NAME L".",
                            L"" PRODUCT_NAME, MB_ICONASTERISK | MB_OK);
            }
        } else {
            if (bIsUpdate) {
                HKEY  hKey   = NULL;
                ...
                if (hKey) {
                    dwSize = TRUE;
                    RegSetValueExW(hKey, L"IsUpdatePending", 0, REG_DWORD, &dwSize, sizeof(DWORD));
                    RegCloseKey(hKey);
                }
            }
    ...
    if (!bOk) { //  && !(argc >= 1 && WStrIEq(wargv[0], L"/update_silent"))
        MessageBoxW(
            NULL,
