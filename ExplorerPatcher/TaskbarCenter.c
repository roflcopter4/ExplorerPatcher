...
#define EP_TASKBAR_LENGTH_PROP_NAME L"EPTBLEN"
#define EP_TASKBAR_LENGTH_TOO_SMALL 20

static BOOL bTaskbarCenterHasPatchedSHWindowsPolicy = FALSE;
static UINT atomPeopleBand = 0;
static UINT atomMSTaskListWClass = 0;
static UINT atomMSTaskSwWClass = 0;
    ...
    if (GetClassWord(hWnd, GCW_ATOM) == atomMSTaskListWClass)
    {
        ...
        if (TaskbarCenter_ShouldCenter(dwSetting))
        {
            if (hWndStart)
            {
                ...
                {
                    if (WStrEq(pCn, L"TrayButton") && !IsWindowVisible(hTrayButton)) continue;
                    RECT rcTrayButton;
                    ...
                    if (WStrEq(pCn, L"TrayDummySearchControl")) {
                        pCn = L"TrayButton";
                        hTrayButton = NULL;
                    }
                }
